...
def ffmpeg_to_wav_24k(in_path: str, out_path: str) -> None:
    run(["ffmpeg", "-y", "-i", in_path, "-ac", "1", "-ar", "24000", "-vn", out_path])


def ffmpeg_trim(in_wav: str, out_wav: str, seconds: int) -> None:
    run(["ffmpeg", "-y", "-i", in_wav, "-t", str(seconds), out_wav])
...
def tts_xtts_clone(text: str, speaker_wav: str, tts_lang: str, out_wav: str):
    tts = get_xtts()
    tts.tts_to_file(text=text, speaker_wav=speaker_wav, language=tts_lang, file_path=out_wav)
...
def handler(event):
    ...
    if not targets:
        raise ValueError("targets is required (list of NLLB language codes)")
    ...
    runpod.serverless.progress_update(event, "Decoding + resampling to WAV 24kHz...")
    ffmpeg_to_wav_24k(in_mp3, in_wav)

    runpod.serverless.progress_update(event, f"Extracting reference ({reference_seconds}s)...")
    ...
    for tgt in targets:
        runpod.serverless.progress_update(event, f"Translating to {tgt} (NLLB)...")
        translated = translate_nllb(transcript, nllb_src_lang, tgt)
        translations[tgt] = translated

        tts_lang = tts_lang_map.get(tgt)
        if not tts_lang:
            raise ValueError(f"Missing tts_lang_map for target {tgt}")

        out_wav = os.path.join(workdir, f"out_{tgt}.wav")
        out_mp3 = os.path.join(workdir, f"out_{tgt}.mp3")

        runpod.serverless.progress_update(event, f"TTS XTTS-v2 (voice clone) for {tgt}...")
        tts_xtts_clone(translated, speaker_wav=ref_wav, tts_lang=tts_lang, out_wav=out_wav)
